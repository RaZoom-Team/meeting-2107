...
class PrometheusMiddleware(BaseHTTPMiddleware):
    def __init__(self, app: ASGIApp, app_name: str = "fastapi-app", exclude_paths: list[str] = []) -> None:
        super().__init__(app)
    ...
    def get_path(self, request: Request) -> Tuple[str, bool]:
        for route in request.app.routes:
            ...
            if match == Match.FULL:
                return route.path, route.path not in self.exclude_paths

        return request.url.path, False


def metrics(request: Request) -> Response:
    if not request.client.host.startswith("172."): raise HTTPException(403)
