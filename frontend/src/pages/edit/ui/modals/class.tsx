...
export function ModalClass({nowClass, is_open, close_hook}: Props) {
    ...
    <div className={styles['content']}>
        <Select width='max' label='Класс:' size="l" value={litera} defaultValue={litera} multiple={false} filterable={true} onUpdate={setLitera}>
            {Literales.map(option => <Select.Option key={option.key} value={option.key}>{option.value}</Select.Option>)}
        </Select>
